...
Require.prototype = {
	...
	makeDefineFn: function(modId, callfn, ifMake){
		if(ifMake === false) {
			return callfn;
		}else {
			return function(){
				var mods = [].concat(module.getMod(modId));

				callfn ? callfn.apply(this, mods) : '';
			};
		}
	},
	load: function(mod, callfn, cloneMods){
		...
		if(type == 'string'){
			this.doLoad(mod, this.makeDefineFn(mod, callfn));
		}else if(type == 'array') {
			//save Names
			var cloneMods = cloneMods || stone.cloneArray(mod);
			var curMod = mod.shift();
			mod.length > 0 ? self.doLoad(curMod, function(){
				self.load(mod, callfn, cloneMods)
			}, false) : self.doLoad(curMod, this.makeDefineFn(cloneMods, callfn));
		}
	},
	doLoad: function(mod, callfn, isLast){
		var jssrc = module.getPath() + stone.getFullName(mod, 'js');
		var modName = stone.getShortName(mod, 'js');
		
		var fns = Require.init;
		//console.log(fns.getSrc());
		//****** debug
		module.inLoading.push(modName);
		module.logObj[modName] = modName;
		//console.log('require: '+modName);

		var debug_callfn = function(){
		...
		!stone.inArray(jssrc, fns.getSrc()) ? stone.getScript(jssrc, debug_callfn) : '';
	},
...
var module = function(){
	...
	exports.queryTask = function(modName){
		var logObj = this.logObj;
		stone.isEmpty(logObj) ? this.runDefine(this.getDefineList()) : '';
	};
...
var define = function(){
	...
	var expfn = function(modName, rMod, deffn){
		//delete module.logObj[modName];
		//console.log(arguments);
		//if rMod Exist:array or string 
		if( stone.inArray(stone.getType(arguments[1]), ['string', 'array']) ){
			//module.preReg[modName] = stone.cloneArray([].concat(rMod));
			//reg in inDefine
			...
			module.inDefine[modName] = {
				...
				deffn: function(){
					...
					//console.log(rMod);
					var modObj = deffn.apply(null, rModObj);
					module.regMod(modName, modObj, rModObj);
				}
			};
			require(rMod, function(args){
				var rModObj = args;
				//module.regMod(modName, deffn, rModObj);
			});
		}else {
			//no rMod, only modName and deffn
			var deffn = arguments[1];
			module.inDefine[modName] = {
				deffn: function(){
					//var rModObj = module.getMod(rMod);
					var modObj = deffn();
					module.regMod(modName, modObj);					
				}
			};
			//module.regMod(modName, deffn );
		}
	};
